...
const UserStoreView = (props) => {
  ...
  useEffect(() => {
    console.log(
      "UserStoreView useEffect 1: Behavior before the component is added to the DOM"
    );
    console.log(
      "UserStoreView.useEffect 1.props.match.params.url",
      props.match.params.url
    );
    console.log(
      "UserStoreView.useEffect 1.props.match.params.id",
  ...
  useEffect(() => {
    console.log(
      "UserStoreView useEffect 3: Behavior before the component is added to the DOM"
    );
    ...
    console.log(
      "UserStoreView.useEffect 3: store.userStore is ?=",
      store.userStore === undefined
    );
    console.log(
      "UserStoreView.useEffect 3: state.userStore is ?=",
  ...
  console.log(
    ">>>>UserStoreView.process.env.REACT_APP_BACK_IMAGES=",
        ...
        if (edit) {
          ...
          return (
            ...
            <ProductCard
              ...
              urlImages={urlImages}
              prod={p}
              addProduct={actions.addProductToCart}
            />
          );
        }
      }
      ...
      <div className="user-store-view">
        <div className="user-store-view-01">
          <div className="user-store-view-01-a">
            <img
              className="user-store-view-01-a-01"
              src={store.userStore && urlImages + store.userStore.user.photoUrl}
            />
            <div className="user-store-view-01-a-02">
              <label className="user-store-view-01-a-02-1">
                {store.userStore && store.userStore.url}
              </label>
            ...
            <div className="user-store-view-01-a-03">
              <label>
                En Chachurero desde{" "}
                {store.userStore && store.userStore.createdAt}
              </label>
            </div>
          </div>
          <div className="user-store-view-01-b">
            <img
              src={store.userStore && urlImages + store.userStore.photoUrl}
            ></img>
          </div>
        </div>
        <div className="user-store-view-02">
          <div className="user-store-view-02-a">
            ...
            <label>
              {store.userStore && store.userStore.followers} seguidores
            </label>
            <label>
              {store.userStore && store.userStore.followeds} seguindo
            </label>
          </div>
        ...
        <div className="user-store-view-03"></div>
        <div className="user-store-view-04">
          <Filter />
        </div>
        <div className="user-store-view-05">{products}</div>
      </div>
    </div>
  );
};
