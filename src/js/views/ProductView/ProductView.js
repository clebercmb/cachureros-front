...
const ProductView = (props) => {
  ...
  useEffect(() => {
    console.log(
      "ProductView useEffect 1: Behavior before the component is added to the DOM"
    );
    console.log(
      "ProductView.useEffect 1.props.match.params.id",
  ...
  useEffect(() => {
    console.log(
      "ProductView useEffect 2: Behavior before the component is added to the DOM"
    );
    console.log("ProductView.useEffect 2.store.product", store.product);

    let newState = state;
    newState.product = store.product;
    newState.product["amount"] = 1;
  ...
  return (
    <div className="productview-container">
        ...
        <div className="productview-a-01">
          <div className="productview-a-01-01">
            {state.product.photos && (
              <ProductLargePhoto
                endpoint={urlImages}
                src={state.product.photos[0]}
                index={0}
                onclick={changePhotoArray}
                alt="Product Photo"
              />
            )}
          </div>
          <div className="productview-a-01-02">
            {state.product.photos && (
              <ProductSmallPhoto
                endpoint={urlImages}
                src={state.product.photos[1]}
                index={1}
                onclick={changePhotoArray}
                alt="Small Product Photo 1"
              />
            )}
            {state.product.photos && (
              <ProductSmallPhoto
                endpoint={urlImages}
                src={state.product.photos[2]}
                index={2}
                onclick={changePhotoArray}
                alt="Small Product Photo 2"
              />
            )}
            {state.product.photos && (
              <ProductSmallPhoto
                endpoint={urlImages}
                src={state.product.photos[3]}
                index={3}
                onclick={changePhotoArray}
                alt="Small Product Photo 3"
              />
            )}
            {state.product.photos && (
              <ProductSmallPhoto
                endpoint={urlImages}
                src={state.product.photos[4]}
                index={4}
                onclick={changePhotoArray}
                alt="Small Product Photo 4"
              />
            )}
          </div>
        </div>
        <div className="productview-a-02">
          <div className="productview-a-02-01">
            ...
            <div className="productview-a-02-01-b">
              <button
                ...
                onClick={(e) => {
                  actions.addProductToCart(state.product, 1);
                }}
              >
                Agregar al Carrito
              </button>
            ...
            <div className="productview-a-02-01-c">
              <label className="productview-a-02-01-c-detail">
                {" "}
          ...
          <div className="productview-a-02-02">
            <div className="userProfile-item-left-item-01">
              <Link
                to={`/user-store/${
                  store.product.userStore && store.product.userStore.url
                }`}
              >
                <img
                  src={
                    store.product.userStore &&
                    urlImages + store.product.userStore.photoUrl
                  }
                  alt="Photo de la tendita"
      ...
      <div className="productview-c">{relatedProducts}</div>
    </div>
  );
};
