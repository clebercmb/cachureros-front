...
const Message = (props) => {
  ...
  useEffect(() => {
    console.log(
      "Message useEffect 1: Behavior before the component is added to the DOM"
    );
    console.log(
      "Message.props.match.params.user_id=",
      props.match.params.user_id
    );
    actions.fetchUserMessages(props.match.params.user_id);
    actions.setInfoBar(true, "Mensajes", actions.getLogin().data.user.name);
  ...
  useEffect(() => {
    console.log(
      "Message useEffect 2: Behavior before the component is added to the DOM"
    );
    let newState = state;
    ...
    userMessages = userMessages.map((p, i) => {
      return (
        <UserMessage
          key={i}
          id={p.id}
          user_from={p.sender.name}
          photo={urlImages + p.sender.photoUrl}
          message={p.message}
          link={p.link}
          status={p.messageStatus.name}
          type={p.messageType.name}
          date={p.createdAt}
  ...
  return <div className="message-container">{userMessages}</div>;
};
